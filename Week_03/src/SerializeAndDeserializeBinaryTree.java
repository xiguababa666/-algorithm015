...
public class SerializeAndDeserializeBinaryTree {
    ...
    public String serialize(TreeNode root) {
        ...
        while (!l.isEmpty() && l.getLast() == null) l.removeLast();
    ...
    public TreeNode deserialize(String data) {
        ...
        for (int i = 0; i < arr.length; i++) {
            ns[i] = "null".equals(arr[i]) ? null : new TreeNode(Integer.parseInt(arr[i]));
        }

        int pos = 0, childPos = 1;
        while (childPos < ns.length) {
            TreeNode n = ns[pos++];
            if (n == null) continue;
            n.left = ns[childPos++];
            if (childPos < ns.length) n.right = ns[childPos++];
        }
    ...
    public TreeNode deserialize2(String data) {
        ...
        for (int i = 0; i < arr.length; i++) {
            TreeNode cur;
            if ("null".equals(arr[i]) || "".equals(arr[i])) {
                cur = null;
            } else {
                cur = new TreeNode(Integer.parseInt(arr[i]));
            }
            nodes.add(pos, cur);
            if (pos == 0) {
                pos++;
            ...
            int parent = (pos - 1) / 2;
            while (nodes.get(parent) == null) {
                nodes.add(pos++, null);
                nodes.add(pos++, null);
                parent++;
            }
            if ((pos - 1) % 2 == 0) {
                nodes.get(parent).left = cur;
            } else {
                nodes.get(parent).right = cur;
            }
            pos++;
        }
        return nodes.get(0);
